    ...
    for u_batch_id in range(n_user_batchs): 
        ...
        if drop_flag == False:
            ...
            for user in user_batch:
                test_U.append(np.full(len(item_batch), user, dtype="int64"))
                test_I.append(item_batch)

            test_U = np.concatenate(test_U, 0)
            test_I = np.concatenate(test_I, 0)


            rate_batch = sess.run(model.batch_ratings, {model.users: test_U,
                                                        model.pos_items: test_I,
                                                        model.neg_items: test_I,
